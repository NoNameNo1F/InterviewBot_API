...
namespace Interviewbot_API.BuildingBlocks.Application.Common.Filters;
    ...
    where TEntity : class
{
    public static Expression<Func<TEntity, bool>>? CreateEqualFilter(IDictionary<string, object> filters)
    {
        ...
        foreach (var pair in filters)
        {
            ...
            var constain = Expression.Constant(pair.Value);
            var expression = Expression.Equal(member, constain);
